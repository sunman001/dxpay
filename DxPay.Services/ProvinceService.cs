...
{
    public class ProvinceService : GenericService<jmp_province>, IProvinceService
    {
        ...
        public jmp_province FindBytime(string start, string end)
        {

            var where = new List<string>();
            if (!string.IsNullOrEmpty(start))
            {
                where.Add("  convert(varchar(10),p_time,120)>='" + start + "' ");
            }
            if (!string.IsNullOrEmpty(end))
            {
              where.Add("convert(varchar(10),p_time,120)<='" + end + "' ");
            }
        ...
        public IEnumerable<jmp_province> FindPagedList(int userid, string orderBy, string stime, string etime, string a_name)
        {
            var where = new List<string>();
            if (!string.IsNullOrEmpty(stime))
            {
               where.Add("convert(varchar(10),a.p_time,120)>='" + stime + "' ");
            }
            if (!string.IsNullOrEmpty(etime))
            {
                where.Add(" convert(varchar(10),a.p_time,120)<='" + etime + "' ");
        ...
        public IEnumerable<jmp_province> FindPagedListByBp(int userid, string orderBy, string stime, string etime, string a_name)
        {
            var where = new List<string>();
            string bpWhere = "";
            string agentWhere = "";
