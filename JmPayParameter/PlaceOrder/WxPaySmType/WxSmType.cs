...
namespace JmPayParameter.PlaceOrder.WxPaySmType
{
    ...
    public class WxSmType
    {
        ...
        /// <returns></returns>
        public InnerResponse WxSmPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, int infoTime, int appid, string ip)
        {
            InnerResponse inn = new InnerResponse();
            if (paymode == 3)
            {
                inn = WxsmH5(apptype, code, price, orderid, goodsname, infoTime, appid, ip);
            }
            else
            {
        ...
        private SelectInterface SelectInfo(string cache, int apptype, int appid, int infoTimes)
        {
            ...
            try
            {
                ...
                if (JMP.TOOL.CacheHelper.IsCache(cache))
                {
                    ...
                    {
                        dt = bll.SelectPay("WXGFSM", apptype, appid);
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            int row = new Random().Next(0, dt.Rows.Count);
                            string[] paypz = dt.Rows[row]["l_str"].ToString().Split(',');
