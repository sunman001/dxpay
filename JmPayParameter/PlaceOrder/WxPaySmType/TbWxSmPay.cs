...
namespace JmPayParameter.PlaceOrder.WxPaySmType
{
    ...
    public class TbWxSmPay
    {
        ...
        /// <returns></returns>
        public InnerResponse TbWxSmPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, string ip, int infoTimes, int appid)
        {
            InnerResponse inn = new InnerResponse();
            if (paymode == 3)
            {
                inn = TbWxSmH5(apptype, code, goodsname, price, orderid, ip, appid, infoTimes);
            }
            else
            {
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private InnerResponse TbWxSmH5(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid, int infoTimes)
        {
            InnerResponse inn = new InnerResponse();
            SelectInterface SeIn = new SelectInterface();
            try
            {
                string TbWxSmH5jkhc = "TbWxSmH5jkhc" + appid;//组装缓存key值

                SeIn = SelectUserInfo(TbWxSmH5jkhc, apptype, appid, infoTimes);
                ...
                list.Add("trade_type", "trade.weixin.native");//交易类型
                ...
                if (dict["return_code"].ToString() == "SUCCESS" && dict["result_code"].ToString() == "SUCCESS")
                {
                    ...
                    string qurl = dict["code_url"].ToString() + "," + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + ",2";//组装二维码地址
                ...
                {
                    string tbwxzfxx = "途贝微信扫码支付错误代码：" + srcString + ",商户号：" + SeIn.UserId;
                    PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + tbwxzfxx, summary: "途贝微信扫码支付接口错误信息", channelId: SeIn.PayId);
            ...
            catch (Exception E)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + E.ToString(), summary: "途贝微信扫码接口错误信息", channelId: SeIn.PayId);
                inn = inn.ToResponse(ErrorCode.Code104);
            }
            return inn;
        }
        ...
        private SelectInterface SelectUserInfo(string cache, int apptype, int appid, int infoTimes)
        {
            ...
            try
            {
                ...
                if (JMP.TOOL.CacheHelper.IsCache(cache))
                {
                    ...
                    else
                    {
                        ...
                        dt = bll.SelectPay("TBWXSM", apptype, appid);
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            int row = new Random().Next(0, dt.Rows.Count);
                            string[] paypz = dt.Rows[row]["l_str"].ToString().Split(',');
                        ...
                        {
                            PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",获取缓存未成功后在次查询数据！", summary: "途贝微信扫码支付支付接口错误", channelId: SeIn.PayId);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",在数据库为查询到数据", summary: "途贝微信扫码支付支付接口错误", channelId: SeIn.PayId);
                    }
                }
            }
            catch (Exception e)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog(bcxx, summary: "途贝微信扫码支付支付接口错误应用类型ID：" + apptype, channelId: SeIn.PayId);
            }
            return SeIn;
        }
