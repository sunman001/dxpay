...
namespace JmPayParameter.PlaceOrder.AlPaySmType
{
    ...
    public class RmAlPaySm
    {
        ...
        /// <returns></returns>
        public  InnerResponse RmZfbsmPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, int infoTime, int appid)
        {
            InnerResponse inn = new InnerResponse();
            if (paymode == 3)
            {
                inn = RmZfbSmH5(apptype, code, price, orderid, goodsname, infoTime, appid);
            }
            else
            {
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private  InnerResponse RmZfbSmH5(int apptype, string code, decimal price, int orderid, string goodsname, int infoTime, int appid)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (JMP.TOOL.CacheHelper.IsCache(RmZfbSmH5))
                    {
                        ...
                        {
                            dt = bll.SelectPay("RMZFBSM", apptype, appid);
                            if (dt != null && dt.Rows.Count > 0)
                            {
                                int row = new Random().Next(0, dt.Rows.Count);
                                string[] paypz = dt.Rows[row]["l_str"].ToString().Split(',');
                                ...
                                minmun = decimal.Parse(dt.Rows[row]["l_minimum"].ToString());//单笔最小支付金额
                                maximum = decimal.Parse(dt.Rows[row]["l_maximum"].ToString());//单笔最大支付金额
                                JMP.TOOL.CacheHelper.CacheObjectLocak<DataTable>(dt, RmZfbSmH5, infoTime);//存入缓存
                            }
                            else
                            {
                                PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ", 获取缓存信息失败后，从数据库未查询到相关信息！", summary: "融梦支付宝扫码接口错误", channelId: pay_id);
                    ...
                    else
                    {
                        ...
                        {
                            PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",直接从数据库未查询到相关信息！", summary: "融梦支付宝扫码接口错误", channelId: pay_id);
                            inn = inn.ToResponse(ErrorCode.Code106);
                            return inn;
                        }
                    }
                ...
                catch (Exception e)
                {
                    ...
                    PayApiDetailErrorLogger.UpstreamPaymentErrorLog(bcxx, summary: "融梦支付宝扫码支付接口错误应用类型ID：" + apptype,channelId:pay_id);
                    inn = inn.ToResponse(ErrorCode.Code106);
                    return inn;
                }
                #endregion
                ...
                if (!string.IsNullOrEmpty(jmpay))
                {
                    ...
                    if (jsonstr != null && jsonstr.Count > 0 && jsonstr.ContainsKey("payParam") && jsonstr["return_code"].ToString() == "0")
                    {
                        ...
                        string qurl = imageurl + "," + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + ",1";//组装二维码地址
                    ...
                    {
                        PayApiDetailErrorLogger.UpstreamPaymentErrorLog("融梦支付宝扫码支付出错，获取到的回传参数：" + jmpay, summary: "融梦支付宝扫码支付接口错误", channelId: pay_id);
            ...
            catch (Exception E)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + E.ToString(), summary: "融梦支付宝扫码接口错误信息", channelId: pay_id);
