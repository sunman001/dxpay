...
namespace JmPayParameter.PlaceOrder.AlPaySmType
{
    ...
    public class WftAlPaySm
    {
        ...
        /// <returns></returns>
        public  InnerResponse WftPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid)
        {
            InnerResponse inn = new InnerResponse();
            if (paymode == 3)
            {
                inn = WftZfbSm(apptype, code, goodsname, price, orderid, ip, appid);
            }
            else
            {
