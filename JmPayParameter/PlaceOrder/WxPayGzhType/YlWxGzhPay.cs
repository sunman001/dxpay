...
namespace JmPayParameter.PlaceOrder.WxPayGzhType
{
    public class YlWxGzhPay
    {
        ...
        /// <returns></returns>
        public InnerResponse ylWxGzhPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, string ip, int infoTimes, int appid)
        {
            InnerResponse inn = new InnerResponse();
            if (paymode == 3)
            {
                inn = YLWxGzhPayH5(apptype, code, goodsname, price, orderid, ip, appid, infoTimes);
            }
            else
            {
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private InnerResponse YLWxGzhPayH5(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid, int infoTimes)
        {
            InnerResponse inn = new InnerResponse();
            SelectInterface SeIn = new SelectInterface();
            try
            {
                string YLWxGzhPayH5 = "YLWxGzhPayH5" + appid;//组装缓存key值

                SeIn = SelectUserInfo(YLWxGzhPayH5, apptype, appid, infoTimes);
                ...
                list.Add("spbill_create_ip", "192.168.1.1"); //终端 IP
                ...
                list.Add("trade_type", "trade.weixin.jspay");//交易类型
                ...
                {
                    string wftzfsbxin = "优乐微信公众号支付错误代码：" + srcString + ",商户号：" + SeIn.UserId + ",订单编号：" + code;
                    PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + wftzfsbxin, summary: "优乐微信公众号支付接口错误信息", channelId: SeIn.PayId);
            ...
            catch (Exception E)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + E.ToString(), summary: "优乐微信公众号接口错误信息", channelId: SeIn.PayId);
                inn = inn.ToResponse(ErrorCode.Code104);
            }
            return inn;
        }
        ...
        private SelectInterface SelectUserInfo(string cache, int apptype, int appid, int infoTimes)
        {
            ...
            try
            {
                ...
                if (JMP.TOOL.CacheHelper.IsCache(cache))
                {
                    ...
                    {
                        dt = bll.SelectPay("YLWXGZH", apptype, appid);
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            int row = new Random().Next(0, dt.Rows.Count);
                            string[] paypz = dt.Rows[row]["l_str"].ToString().Split(',');
                        ...
                        {
                            PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",获取缓存失败后，从数据库未查询到相关信息！", summary: "优乐微信公众号支付支付接口错误", channelId: SeIn.PayId);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",从数据库未查询到相关信息！", summary: "优乐微信公众号支付支付接口错误", channelId: SeIn.PayId);
                    }
                }
            }
            catch (Exception e)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog(bcxx, summary: "优乐微信公众号支付支付接口错误应用类型ID：" + apptype, channelId: SeIn.PayId);
            }
            return SeIn;
        }
    }
