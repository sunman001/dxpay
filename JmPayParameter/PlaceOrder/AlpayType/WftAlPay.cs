...
namespace JmPayParameter.PlaceOrder.AlpayType
{
    ...
    public class WftAlPay
    {
        ...
        /// <returns></returns>
        public InnerResponse WftZFBPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid)
        {
            ...
            {
                case 1://安卓方式
                    inn = WftZfbAz(apptype, code, goodsname, price, orderid, ip, appid);
                    break;
                case 2://ios方式
                    inn = WftZfbIOS(apptype, code, goodsname, price, orderid, ip, appid);
                    break;
                case 3://H5方式
                    inn = WftZfbH5(apptype, code, goodsname, price, orderid, ip, appid);
                    break;
                default:
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private static InnerResponse WftZfbIOS(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid)
        {
            // string str = "";
            InnerResponse inn = new InnerResponse();
            Dictionary<string, string> cfg = new Dictionary<string, string>();
            try
            {
                ...
                if (pay.call())
                {
                    ...
                    if (resHandler.isTenpaySign())
                    {
                        if (int.Parse(param["status"].ToString()) == 0 && int.Parse(param["result_code"].ToString()) == 0)
                        {
                            string Alpay = "{\"PaymentType\":\"1\",\"SubType\":\"4\",\"IsH5\":\"1\",\"data\":\"" + param["code_url"] + "\"}";
                            inn = inn.ToResponse(ErrorCode.Code100);
                        ...
                        {
                            string wftzfsbxin = "威富通支付失败信息，错误代码：" + param["err_code"] + ",错误信息：" + param["err_msg"] + ",商户号：" + cfg["mch_id"].ToString();
                            PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + wftzfsbxin, summary: "威富通支付宝wap接口错误信息", channelId: int.Parse(cfg["pay_id"].ToString()));
                            inn = inn.ToResponse(ErrorCode.Code104);
                        }
                    }
                    else
                    {
                        string mesage = "威富通支付失败信息，错误代码：" + resHandler.getContent() + ",错误信息：" + resHandler.getDebugInfo() + ",商户号：" + cfg["mch_id"].ToString();
                        PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：" + mesage, summary: "威富通支付宝wap接口错误信息", channelId: int.Parse(cfg["pay_id"].ToString()));
                        inn = inn.ToResponse(ErrorCode.Code104);
                    }
                }
                else
                {
                    ...
                    PayApiDetailErrorLogger.UpstreamPaymentErrorLog("报错信息：第一步验证错误", summary: "威富通支付宝wap接口错误信息", channelId: int.Parse(cfg["pay_id"].ToString()));
                    inn = inn.ToResponse(ErrorCode.Code104);
                }
            }
            catch (Exception E)
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private static InnerResponse WftZfbAz(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid)
        {
            // string str = "";
            InnerResponse inn = new InnerResponse();
            Dictionary<string, string> cfg = new Dictionary<string, string>();
