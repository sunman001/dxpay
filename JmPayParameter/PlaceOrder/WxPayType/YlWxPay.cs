...
namespace JmPayParameter.PlaceOrder.WxPayType
{
    public class YlWxPay
    {
        ...
        /// <returns></returns>
        public InnerResponse ylWxPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, string ip, int infoTimes, int appid)
        {
        ...
        private InnerResponse YLIos(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid, int infoTimes)
        {
            ...
            try
            {
                ...
                if (dict["return_code"].ToString() == "SUCCESS" && dict["result_code"].ToString() == "SUCCESS")
                {
                    string wxpay = "{\"data\":\"" + dict["prepay_url"].ToString() + "\",\"prepay_id\":\"" + dict["prepay_id"] + "\",\"PaymentType\":\"2\",\"SubType\":\"3\",\"IsH5\":\"1\"}";
                    inn = inn.ToResponse(ErrorCode.Code100);
        ...
        private SelectInterface SelectUserInfo(string cache, int apptype, int appid, int infoTimes)
        {
            ...
            try
            {
                ...
                if (JMP.TOOL.CacheHelper.IsCache(cache))
                {
                    ...
                    else
                    {
                        ...
                        dt = bll.SelectPay("YLWXWAP", apptype, appid);
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            int row = new Random().Next(0, dt.Rows.Count);
                            string[] paypz = dt.Rows[row]["l_str"].ToString().Split(',');
                        ...
                        {
                            PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",获取缓存失败后，从数据库未查询到相关信息！", summary: "优乐微信wap支付支付接口错误", channelId: SeIn.PayId);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",从数据库未查询到相关信息！", summary: "优乐微信wap支付支付接口错误", channelId: SeIn.PayId);
                    }
                }
            }
            catch (Exception e)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog(bcxx, summary: "优乐微信wap支付支付接口错误应用类型ID：" + apptype, channelId: SeIn.PayId);
            }
            return SeIn;
        }
