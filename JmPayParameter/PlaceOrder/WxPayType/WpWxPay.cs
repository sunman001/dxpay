...
namespace JmPayParameter.PlaceOrder.WxPayType
{
    ...
    public class WpWxPay
    {
        ...
        /// <returns></returns>
        public InnerResponse WpWxPayInfo(int paymode, int apptype, string code, string goodsname, decimal price, int orderid, string ip, int infoTimes, int appid)
        {
            InnerResponse inn = new InnerResponse();
            switch (paymode)
            {
                case 1://安卓方式
                    inn = WpWxWaPAz(apptype, code, goodsname, price, orderid, ip, appid, infoTimes);
                    break;
                case 2://ios方式
                    inn = WpWxWaPIOS(apptype, code, goodsname, price, orderid, ip, appid, infoTimes);
                    break;
                case 3://H5支付方式
                    inn = WpWxWaPH5(apptype, code, goodsname, price, orderid, ip, appid, infoTimes);
                    break;
                default:
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private InnerResponse WpWxWaPAz(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid, int infoTimes)
        {
            InnerResponse inn = new InnerResponse();
            SelectInterface SeIn = new SelectInterface();
            try
            {
                ...
                try
                {
                    ...
                    if (dict["resultCode"].ToString() == "success")
                    {
                        string wxpay = "{\"data\":\"" + dict["url"].ToString() + "\",\"PaymentType\":\"2\",\"SubType\":\"7\",\"IsH5\":\"1\"}";
                        inn = inn.ToResponse(ErrorCode.Code100);
        ...
        /// <param name="appid">应用id</param>
        /// <returns></returns>
        private InnerResponse WpWxWaPIOS(int apptype, string code, string goodsname, decimal price, int orderid, string ip, int appid, int infoTimes)
        {
            InnerResponse inn = new InnerResponse();
            SelectInterface SeIn = new SelectInterface();
            try
        ...
        private SelectInterface SelectUserInfo(string cache, int apptype, int appid, int infoTimes)
        {
            ...
            try
            {
                ...
                if (JMP.TOOL.CacheHelper.IsCache(cache))
                {
                    ...
                    else
                    {
                        ...
                        dt = bll.SelectPay("WPWXWAP", apptype, appid);
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            int row = new Random().Next(0, dt.Rows.Count);
                            string[] paypz = dt.Rows[row]["l_str"].ToString().Split(',');
                        ...
                        {
                            PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",获取缓存未成功后在次查询数据！", summary: "微派微信wap支付支付接口错误", channelId: SeIn.PayId);
                        }
                    }
                }
                else
                {
                    ...
                    {
                        PayApiDetailErrorLogger.UpstreamPaymentErrorLog("应用id为：" + appid + "的支付通道为空！风控id为：" + apptype + ",在数据库为查询到数据", summary: "微派微信wap支付支付接口错误", channelId: SeIn.PayId);
                    }
                }
            }
            catch (Exception e)
            {
                ...
                PayApiDetailErrorLogger.UpstreamPaymentErrorLog(bcxx, summary: "微派微信wap支付支付接口错误应用类型ID：" + apptype, channelId: SeIn.PayId);
            }
            return SeIn;
        }
