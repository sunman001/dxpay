...
function SaveUser() {
    ...
    if (isImgNull) {
        ModifyTipCss("u_photo_tip", "请选择身份证正面照片！");
        tmsg += "请选择身份证照片！";
        return false;
    } else if (!isImg) {
    ...
    if (stype == "1") {
        ...
        if (isImgNull) {
            ModifyTipCss("sfzficateyy", "请选择营业执照照片！");
            tmsg += "请选择证件图片！";
            return false;
        } else if (!isImg) {
            ModifyTipCss("sfzficateyy", "请选择营业执照照片（.jpg.png.bmp.jpeg）！");
            tmsg += "请选择正确格式的证件图片！";
            return false;
        }
    }
...
function SaveUpdateUser() {
    ...
    var phone = fd.u_phone;
    if (!isMobile(phone)) {
        ModifyTipCss("u_phone_tip", "请填写正确的11位手机号！");
        tmsg += "11位手机号";
        return false;
    }
...
function FileChangexkz()
{
    ...
    if (filedata.length > 0) {
        ...
        if (isImg) {
            ...
            window.parent.ShowMsg("证件照片（.jpg.png.bmp.jpeg）！", "error", "");
            ModifyTipCss("u_licence_tip", "证件照片（.jpg.png.bmp.jpeg）！");
        }
    } else {
        ModifyTipCss("u_licence_tip", "证件照片！");
    }
}
