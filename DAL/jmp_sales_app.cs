...
namespace JMP.DAL
{
    ...
    public partial class jmp_sales_app
    {
        public bool Exists(int r_id, int r_appid, decimal r_moneys, DateTime r_date)
        {
            ...
            strSql.Append("select count(1) from jmp_sales_app");
            strSql.Append(" where ");
            strSql.Append(" r_id = @r_id and  ");
            strSql.Append(" r_appid = @r_appid and  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into jmp_sales_app(");
            strSql.Append("r_appid,r_moneys,r_date");
            ...
            parameters[0].Value = model.r_appid;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update jmp_sales_app set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from jmp_sales_app ");
            strSql.Append(" where r_id=@r_id");
            SqlParameter[] parameters = {
			    new SqlParameter("@r_id", SqlDbType.Int,4)
			};
        ...
        public JMP.MDL.jmp_sales_app GetModel(int r_id)
        {
            ...
            strSql.Append("  from jmp_sales_app ");
            strSql.Append(" where r_id=@r_id");
            SqlParameter[] parameters = {
				new SqlParameter("@r_id", SqlDbType.Int,4)
			};
            ...
            JMP.MDL.jmp_sales_app model = new JMP.MDL.jmp_sales_app();
        ...
        public DataTable GetLists(string start, string end, string sType, string sKey, int tops)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("select " + (tops > 0 ? "top " + tops : "") + " b_app.a_name as tname,temp.r_moneys from (");
            strSql.AppendLine("    select r_appid,sum(r_moneys) r_moneys from jmp_sales_app");
            ...
            if (!string.IsNullOrEmpty(sType) && !string.IsNullOrEmpty(sKey))
            {
                ...
                {
                    case "0":
                        strSql.AppendFormat(" and b_app.a_name='{0}'", sKey);
                        break;
                    case "1":
                        strSql.AppendFormat(" and b_user.u_email='{0}'", sKey);
                        break;
                }
            }
            strSql.AppendLine("order by temp.r_moneys desc");
        ...
        public DataTable GetListsUser(string start, string end, string uid, string aid, int tops)
        {
            ...
            strSql.AppendLine("left join [" + BsaeDb + "].[dbo].[jmp_app] b_app on temp.r_appid=b_app.a_id");
            strSql.AppendLine("left join [" + BsaeDb + "].[dbo].[jmp_user] b_user on b_app.a_user_id=b_user.u_id");
            strSql.Append("where b_user.u_id=" + uid);
