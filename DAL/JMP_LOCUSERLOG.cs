...
namespace JMP.DAL  
{
		...
		public partial class jmp_locuserlog
	{
		...
		public bool Exists(int l_id)
		{
			...
			strSql.Append("select count(1) from jmp_locuserlog");
			strSql.Append(" where ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into jmp_locuserlog(");			
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update jmp_locuserlog set ");
...
SqlParameter[] parameters = {
		...
		public bool Delete(int l_id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from jmp_locuserlog ");
			strSql.Append(" where l_id=@l_id");
						SqlParameter[] parameters = {
					new SqlParameter("@l_id", SqlDbType.Int,4)
			};
		...
		public JMP.MDL.jmp_locuserlog GetModel(int l_id)
		{
			...
			strSql.Append("  from jmp_locuserlog ");
			strSql.Append(" where l_id=@l_id");
						SqlParameter[] parameters = {
					new SqlParameter("@l_id", SqlDbType.Int,4)
			};
			...
			JMP.MDL.jmp_locuserlog model=new JMP.MDL.jmp_locuserlog();
        ...
        public List<JMP.MDL.jmp_locuserlog> SelectList(string sqls, string Order, int pageIndexs, int PageSize, out int pageCount)
        {
            ...
            da.Dispose();
            return DbHelperSQL.ToList<JMP.MDL.jmp_locuserlog>(ds.Tables[0]);
        }
