...
{
    //销售排行（渠道）
    public partial class jmp_sales_pay
    {
        public bool Exists(int r_id, int r_paymode, decimal r_moneys, DateTime r_date)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from jmp_sales_pay");
            strSql.Append(" where ");
            strSql.Append(" r_id = @r_id and  ");
            strSql.Append(" r_paymode = @r_paymode and  ");
            strSql.Append(" r_moneys = @r_moneys and  ");
            strSql.Append(" r_date = @r_date  ");
            SqlParameter[] parameters = {
				new SqlParameter("@r_id", SqlDbType.Int,4),
				new SqlParameter("@r_paymode", SqlDbType.Int,4),
				new SqlParameter("@r_moneys", SqlDbType.Money,8),
				new SqlParameter("@r_date", SqlDbType.DateTime)
            };
            parameters[0].Value = r_id;
            parameters[1].Value = r_paymode;
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(JMP.MDL.jmp_sales_pay model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into jmp_sales_pay(");
            strSql.Append("r_paymode,r_moneys,r_date,r_appid");
            ...
            parameters[0].Value = model.r_paymode;
            parameters[1].Value = model.r_moneys;
            parameters[2].Value = model.r_date;
            parameters[3].Value = model.r_appid;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(JMP.MDL.jmp_sales_pay model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update jmp_sales_pay set ");

            strSql.Append("r_paymode=@r_paymode,");
            ...
            SqlParameter[] parameters = {
                ...
                new SqlParameter("@r_moneys", SqlDbType.Money,8),            
                new SqlParameter("@r_date", SqlDbType.DateTime),
                new SqlParameter("@r_appid", SqlDbType.DateTime)
            };

            parameters[0].Value = model.r_id;
            parameters[1].Value = model.r_paymode;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from jmp_sales_pay ");
            strSql.Append(" where r_id=@r_id");
            SqlParameter[] parameters = {
				new SqlParameter("@r_id", SqlDbType.Int,4)
			};
        ...
        /// 得到一个对象实体
        /// </summary>
        public JMP.MDL.jmp_sales_pay GetModel(int r_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select r_id,r_paymode,r_moneys,r_date,r_appid");
            strSql.Append("  from jmp_sales_pay ");
            strSql.Append(" where r_id=@r_id");
            SqlParameter[] parameters = {
				new SqlParameter("@r_id", SqlDbType.Int,4)
			};
            ...
            JMP.MDL.jmp_sales_pay model = new JMP.MDL.jmp_sales_pay();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["r_paymode"].ToString() != "")
                {
                    model.r_paymode = int.Parse(ds.Tables[0].Rows[0]["r_paymode"].ToString());
                }
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" r_id,r_paymode,r_moneys,r_date,r_appid ");
            strSql.Append(" FROM jmp_sales_pay ");
            if (!string.IsNullOrEmpty(strWhere))
                strSql.Append(" where " + strWhere);
        ...
        public DataTable GetLists(string start, string end, string sType, string sKey, int tops)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("select " + (tops > 0 ? "top " + tops : "") + " b_pm.p_name as tname,sum(temp.r_moneys) r_moneys from (");
            strSql.AppendLine("    select r_appid,r_paymode,sum(r_moneys) r_moneys from jmp_sales_pay");
            strSql.AppendLine("    where convert(nvarchar(10),r_date,120)>='" + start + "'");
            strSql.AppendLine("    and convert(nvarchar(10),r_date,120)<='" + end + "'");
            strSql.AppendLine("    group by r_appid,r_paymode");
            strSql.AppendLine(") temp");
            strSql.AppendLine("left join [" + BsaeDb + "].[dbo].[jmp_paymode] b_pm on temp.r_paymode=b_pm.p_id");
            ...
            strSql.AppendLine("group by b_pm.p_name");
