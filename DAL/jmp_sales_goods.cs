...
namespace JMP.DAL
{
    ...
    public partial class jmp_sales_goods
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into jmp_sales_goods(");
            strSql.Append("r_goodid,r_moneys,r_date,r_appid");
            ...
            parameters[0].Value = model.r_goodid;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update jmp_sales_goods set ");

            strSql.Append("r_goodid=@r_goodid,");
            ...
            parameters[1].Value = model.r_goodid;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from jmp_sales_goods ");
            strSql.Append(" where r_id=@r_id");
            SqlParameter[] parameters = {
				new SqlParameter("@r_id", SqlDbType.Int,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select r_id,r_goodid,r_moneys,r_date,r_appid");
            strSql.Append("  from jmp_sales_goods ");
            strSql.Append(" where r_id=@r_id");
            SqlParameter[] parameters = {
				new SqlParameter("@r_id", SqlDbType.Int,4)
			};
            ...
            JMP.MDL.jmp_sales_goods model = new JMP.MDL.jmp_sales_goods();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["r_goodid"].ToString() != "")
                {
                    model.r_goodid = int.Parse(ds.Tables[0].Rows[0]["r_goodid"].ToString());
                }
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" r_id,r_goodid,r_moneys,r_date,r_appid ");
            strSql.Append(" FROM jmp_sales_goods ");
            if (!string.IsNullOrEmpty(strWhere))
                strSql.Append(" where " + strWhere);
        ...
        public DataTable GetLists(string start, string end, string sType, string sKey, int tops)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("select " + (tops > 0 ? "top " + tops + " " : " ") + "b_good.g_name as tname,sum(temp.r_moneys) r_moneys from (");
            strSql.AppendLine("    select r_goodid,sum(r_moneys) r_moneys from jmp_sales_goods");
            strSql.AppendLine("    where convert(nvarchar(10),r_date,120)>='" + start + "'");
            strSql.AppendLine("    and convert(nvarchar(10),r_date,120)<='" + end + "'");
            strSql.AppendLine("    group by r_goodid");
            strSql.AppendLine(") temp");
            strSql.AppendLine("left join [" + BsaeDb + "].[dbo].[jmp_goods] b_good on temp.r_goodid=b_good.g_id");
            strSql.AppendLine("left join [" + BsaeDb + "].[dbo].[jmp_app] b_app on b_good.g_app_id=b_app.a_id");
            ...
            strSql.AppendLine("group by b_good.g_name");
        ...
        public DataTable GetListsUser(string start, string end, string uid, string aid, int tops)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("select " + (tops > 0 ? "top " + tops : "") + " b_good.g_name as tname,sum(temp.r_moneys) r_moneys from (");
            strSql.AppendLine("    select r_appid,r_goodid,sum(r_moneys) r_moneys from jmp_sales_goods");
            strSql.AppendLine("    where convert(nvarchar(10),r_date,120)>='" + start + "'");
            strSql.AppendLine("    and convert(nvarchar(10),r_date,120)<='" + end + "'");
            strSql.AppendLine("    group by r_appid,r_goodid");
            strSql.AppendLine(") temp");
            strSql.AppendLine("left join [" + BsaeDb + "].[dbo].[jmp_goods] b_good on temp.r_goodid=b_good.g_id");
