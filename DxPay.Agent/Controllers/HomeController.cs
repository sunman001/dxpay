...
namespace DxPay.Agent.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Default()
        {
            ...
            //根据日期查询交易金额和笔数（今天）
            model = _AppCountService.DataAppcountAdyAgent(u_time, u_id);

            //根据不同日期统计查询(昨天)
            comodel_yesterday = _CoSettlementDeveloperAppDetailsServic.FindPagedListByDeveloperKFZAgent(u_id, u_time_yesterday, 0);
            //根据不同日期统计查询(本月)
            comodel_month = _CoSettlementDeveloperAppDetailsServic.FindPagedListByDeveloperKFZAgent(u_id, u_time_month, 1);
            //根据不同日期统计查询(上月)
            comodel_preceding_month = _CoSettlementDeveloperAppDetailsServic.FindPagedListByDeveloperKFZAgent(u_id, u_time_preceding_month, 1);
        ...
        public ContentResult HomeCharts(string days)
        {
            ...
            //查询数据
            DataSet ds = _AppCountService.FindPagedListAgent(userid, startTime, endTime, startTimeAdy, endTimeAdy);
