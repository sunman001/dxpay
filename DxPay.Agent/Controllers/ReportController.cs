...
namespace DxPay.Bp.Controllers
{
    public class ReportController : Controller
    {
        ...
        public ActionResult OrderList()
        {
            ...
            var list = _OrderService.FindPagedListByAgent(userid,"", searchType, searchname, stime, etime, paymode, paymentstate, noticestate, null, pageIndexs, PageSize);
        ...
        public ActionResult AppReport(string rtype)
        {
            ...
            if (rtype == "total")
            {
                ViewBag.stime = stime;
                ViewBag.etime = etime;
                var list = _UserReportService.FindPagedListByAgentTotal(userid, "", rtype, stime, etime, time, num, types, searchKey, sort, searchTotal, null, pageIndexs, PageSize);
            ...
            {
                var list = _UserReportService.FindPagedListByAgentToday(userid, "", rtype, stime, etime, time, num, types, searchKey, sort, searchTotal, null, pageIndexs, PageSize);
                var gridModel = new DataSource<jmp_user_report>(list)
                {
                    Data = list.Select(x => x).ToList()
                };
