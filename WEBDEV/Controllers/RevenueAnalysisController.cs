...
namespace WEBDEV.Controllers
{
    ...
    public class RevenueAnalysisController : Controller
    {
        ...
        public string TradeMain()
        {
            ...
            string appid = !string.IsNullOrEmpty(Request["appid"]) ? Request["appid"] : "0";
            #region 查询数据
            DataTable dt = new DataTable();
         
                dt = bll.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid);
        ...
        public string SalesTopTen()
        {
            ...
            switch (sumType)
            {
                case "0"://应用
                    ...
                    tab = bll_sales_app.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid, 10);
                    dt = bll_sales_app.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid);
                    break;
                case "1"://商户
                    JMP.BLL.jmp_sales_user bll_sales_user = new JMP.BLL.jmp_sales_user();
                    tab = bll_sales_user.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid, 10);
                    dt = bll_sales_user.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid);
                    break;
                case "2"://渠道                    
                    JMP.BLL.jmp_sales_pay bll_sales_pay = new JMP.BLL.jmp_sales_pay();
                    tab = bll_sales_pay.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid, 10);
                    dt = bll_sales_pay.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid);
                    break;
                case "3"://商品
                    JMP.BLL.jmp_sales_goods bll_sales_good = new JMP.BLL.jmp_sales_goods();
                    tab = bll_sales_good.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid, 10);
                    dt = bll_sales_good.GetListsUser(begin, end, UserInfo.UserId.ToString(), appid);
                    break;
            }
            #endregion
            string htmls = string.Empty;
