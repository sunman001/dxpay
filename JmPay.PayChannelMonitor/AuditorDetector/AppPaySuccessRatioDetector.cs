...
namespace JmPay.PayChannelMonitor.AuditorDetector
{
    ...
    /// 应用支付成功率异常监控器
    /// </summary>
    public class AppPaySuccessRatioDetector
    {
        ...
        public void Run()
        {
            ...
            foreach (var appReq in requests)
            {
                ...
                var auditor = new JMP.TOOL.Auditor.JmpAppRequestAuditor(appReq.AppId, appReq.AppName, string.Format("应用【{0}】支付异常:总请求数从前三天的:{1}变化为今日的:{2},成功数从前三天的:{3}变化为今日的:{4},支付成功率从前三天的:{5}%变化为:{6}%,衰减率:{7}%", appReq.AppName, appReq.FirstThreeDaysTotalRequest, appReq.TodayTotalRequest, appReq.FirstThreeDaysPaySuccess, appReq.TodayPaySuccess, appReq.FirstThreeDaysSuccessRatio, appReq.TodaySuccessRatio, Math.Abs((decimal)appReq.SuccessAttenuation)));
                auditor.Add();
            }
        }
