...
namespace JmPay.PayChannelMonitor.MonitorCentre.App
{
    ...
    /// 通道XX分钟无订单的监控
    /// </summary>
    public class AmountSuccessRatioMonitor : MonitorAbstract<AppAmountSuccessRatioSinceLatest>
    {
        /// <summary>
        /// 存储过程名称
        /// </summary>
        ...
        private List<AppAmountSuccessRatioSinceLatest> _data;
        ...
        public AmountSuccessRatioMonitor(string procName, ScheduleGroupCode groupCode) : base(groupCode)
        {
            _procName = procName;
        ...
        protected override void LoadData(IDataLoader dataLoader)
        {
            ...
            _data = DbHelperSQL.ConvertToList<AppAmountSuccessRatioSinceLatest>(ds.Tables[0]).ToList();
        ...
        protected override void Persistent()
        {
            foreach (var o in AllowSenders)
            {
                ...
                bll.Add(new JMP.MDL.jmp_app_request_audit
                {
                    ...
                    message = string.Format("应用[{0}:{1}]金额成功率异常,当前{2}分钟内的成功率为:{3},设定的阀值为:{4},请核实!", o.AppId, o.AppName, o.Minutes, o.SuccessRatio, o.ThresholdSuccessRatio)
                });
            }
        ...
        {
            var filter = new ForAppAmountSuccessRatioFilter(_data);
            filter.DoFilter();
        ...
        protected override void BuildMessageContent()
        {
            ...
            if (ScheduleGroup.MessageTemplate == null)
            {
                ...
                {
                    _message = string.Format("支付平台应用:[{0}] {1}个应用在设定时间内的金额成功率异常,请核实！", string.Join(",", AllowSenders.Select(x => string.Format("{0}:{1}", x.AppId, x.AppName))), AllowSenders.Count);
                }
                else if (AllowSenders.Count > 5)
                {
                    _message = string.Format("支付平台应用:[{0}]...等{1}个应用在设定时间内的金额成功率异常,请核实！", string.Join(",", AllowSenders.Take(5).Select(x => x.AppName)), AllowSenders.Count);
                }
            }
            else
            {
