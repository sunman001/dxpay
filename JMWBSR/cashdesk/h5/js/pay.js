	...
	$.ajax4lock({
		...
		success : function(data) {
			...
			try {
				...
				mInvoke(params, ot, channelKey, inv, payParm, false);
...
function _invoke_WEB(params, ot, channelKey, inv, payParam, isRechr) {
	if (!payParam) {
		alert("支付方式暂不可用（商户通道配置异常）。请联系商户或客服处理。");
		return;
	}
	var param = JSON.parse(payParam);
	if (param.type && param.type.toUpperCase() == "GET") {
		_doget(param);
	} else {
		_dopost(param);
	}
...
function _invoke_WEB_wxwap(tt_params, ot, channelKey, inv, payParam, isRechr) {
	if (!payParam) {
		alert("支付方式暂不可用（商户通道配置异常）。请联系商户或客服处理。");
		return;
	}
	var params = JSON.parse(payParam);
	var wxurl = params.url;
	var r = /^weixin:/i;
	var httpR = /^http/i;
	var wx154R = /^https:\/\/wx.tenpay.com/i;
	if (r.test(wxurl)) {
		wxAbConfirm(wxurl, tt_params.tt, ot, isRechr);
		if ('qh360' == isHrefOpenWx() && ipay.utils.browser.isIOS()) {
			_ifr(wxurl);
		} else {
			top.location.href = wxurl;
		}
	} else if (httpR.test(wxurl)) {
		if (!ipay.utils.isInIframe() && wx154R.test(wxurl)) {
			var getParams = {
			...
			getParams['openSafari'] = ('safari' == isHrefOpenWx());
...
// 处理qq钱包的情况
function _invoke_WEB_qqwap(tt_params, ot, channelKey, inv, payParam, isRechr) {
	if (!payParam) {
		alert("支付方式暂不可用（商户通道配置异常）。请联系商户或客服处理。");
		return;
	}
	var params = JSON.parse(payParam);
	var qqurl = params.url;
	qqAbConfirm(qqurl, tt_params.tt, ot, isRechr);
	var httpR = /^http/i;
	var qq173R = /^https:\/\/myun.tenpay.com/i;
	if (httpR.test(qqurl)) {
		if (!ipay.utils.isInIframe() && qq173R.test(qqurl)) {
			var getParams = {
...
function _invoke_WEB_aliwap(tt_params, ot, channelKey, inv, payParam, isRechr) {
	if (!payParam) {
		alert("支付方式暂不可用（商户通道配置异常）。请联系商户或客服处理。");
		return;
	}
	...
	if (ipay.utils.isWXBrowser()) {
		...
		_invoke_WEB(tt_params, ot, channelKey, inv, payParam, isRechr);
	}
}
...
function wxAbConfirm(wxurl, tt, ot, isRechr) {
...
function qqAbConfirm(qqurl, tt, ot, isRechr) {
	var qqConfirm = function() {
		$.unmask();
		$.abAlert({
			showCloseBtn : true,
			showOk : false, // 点击确定按钮
			...
			butTxt : '付款已完成',
			showFooter : true,
			footerTxt : '无法打开QQ钱包支付？<span class="callWX"  style="color: #0c6cc5;text-decoration:underline;">点此尝试</span>',
			onFooter : function() {
				window.open(qqurl);
			},
			onButton : function() {
