...
{
    //用户日志表
    public partial class jmp_userlog
    {

        private readonly JMP.DAL.jmp_userlog dal = new JMP.DAL.jmp_userlog();
        public jmp_userlog()
        { }
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(JMP.MDL.jmp_userlog model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(JMP.MDL.jmp_userlog model)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public JMP.MDL.jmp_userlog GetModel(int l_id)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_userlog> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_userlog> DataTableToList(DataTable dt)
        {
            List<JMP.MDL.jmp_userlog> modelList = new List<JMP.MDL.jmp_userlog>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                JMP.MDL.jmp_userlog model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new JMP.MDL.jmp_userlog();
