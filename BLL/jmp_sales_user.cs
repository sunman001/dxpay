...
namespace JMP.BLL
{
    ...
    {
        private readonly JMP.DAL.jmp_sales_user dal = new JMP.DAL.jmp_sales_user();
        public jmp_sales_user()
        { }
        ...
        {
            return dal.Exists(r_id, r_userid, r_moneys, r_date);
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_sales_user> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_sales_user> DataTableToList(DataTable dt)
        {
            List<JMP.MDL.jmp_sales_user> modelList = new List<JMP.MDL.jmp_sales_user>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                JMP.MDL.jmp_sales_user model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new JMP.MDL.jmp_sales_user();
