...
namespace JMP.BLL
{
    ...
    {
        private readonly JMP.DAL.jmp_sales_pay dal = new JMP.DAL.jmp_sales_pay();
        public jmp_sales_pay()
        { }
        ...
        {
            return dal.Exists(r_id, r_paymode, r_moneys, r_date);
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_sales_pay> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_sales_pay> DataTableToList(DataTable dt)
        {
            List<JMP.MDL.jmp_sales_pay> modelList = new List<JMP.MDL.jmp_sales_pay>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                JMP.MDL.jmp_sales_pay model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new JMP.MDL.jmp_sales_pay();
                    if (dt.Rows[n]["r_id"].ToString() != "")
                    {
                        model.r_id = int.Parse(dt.Rows[n]["r_id"].ToString());
                    }
                    if (dt.Rows[n]["r_paymode"].ToString() != "")
                    {
                        model.r_paymode = int.Parse(dt.Rows[n]["r_paymode"].ToString());
                    }
