...
namespace JMP.BLL
{
    ...
    {
        private readonly JMP.DAL.jmp_revenue_add dal = new JMP.DAL.jmp_revenue_add();
        public jmp_revenue_add()
        { }
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_revenue_add> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<JMP.MDL.jmp_revenue_add> DataTableToList(DataTable dt)
        {
            List<JMP.MDL.jmp_revenue_add> modelList = new List<JMP.MDL.jmp_revenue_add>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                JMP.MDL.jmp_revenue_add model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new JMP.MDL.jmp_revenue_add();
