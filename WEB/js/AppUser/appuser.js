...
function ChangeType(obj) {
    ...
    if (stext != "" || stext != null || stext != undefined) {
        if (stext == "个人") {
            ...
            document.getElementById('u_licence_tip').innerHTML = '*请选择银行卡正面照片（.jpg.png.bmp.jpeg）！';
            $("#qysfz").hide();
        } else {
            ...
            $('#u_licencehtml').html("开户行许可证照片");
            document.getElementById('u_licencehtml').innerHTML = '开户许可证照片';
            document.getElementById('u_licence_tip').innerHTML = '请选择开户许可证照片（.jpg.png.bmp.jpeg）！';
            $("#qysfz").show();
        }
    }
}
...
function SaveUser() {
    ...
    //获取证件照片的路径
    var isImgNull = isRequestNotNull(fd.u_photo);
    var isImg = CheckImgExtName(fd.u_photo);
    if (isImgNull) {
    ...
    if (isImgNullf) {
        ...
        tmsg += "请选择正确格式的身份证反面图片！";
    }


    //获取开户许可证照片的路径
    var isImgNulll = isRequestNotNull(fd.u_licence);
    var isImgl = CheckImgExtName(fd.u_licence);
    if (isImgNulll) {
        ModifyTipCss("u_licence_tip", "请选择照片！");
        tmsg += "请选择图片！";
    } else if (!isImgl) {
        ModifyTipCss("u_licence_tip", "请选择照片（.jpg.png.bmp.jpeg）！");
        tmsg += "请选择正确格式的照片！";
    }
    if (stype == "1") {
        //获取企业身份证上传
    ...
    if (tmsg == "") {
        ...
        window.parent.ShowMsg("有必填项没有填写！", "error", "");
        $("#btn-save-add").removeAttr("disabled");
        return false;
    }
...
function UpdateFrozenMoney(obj) {
    window.parent.ShouwDiaLogWan("设置冻结金额", 650, 200, "/AppUser/UpdateFrozenMoney?uid=" + obj);
}
//保存用户（编辑）
...
function FileChange() {
    ...
    if (filedata.length > 0) {
        ...
        if (isImg) {
            ...
            window.parent.ShowMsg("请选择身份证正面图片（.jpg.png.bmp.jpeg）！", "error", "");
            ModifyTipCss("u_photo_tip", "请选择身份证正面图片（.jpg.png.bmp.jpeg）！");
        }
    } else {
...
function FileChangexkz() {
    ...
    if (filedata.length > 0) {
        ...
        if (isImg) {
            ...
            window.parent.ShowMsg("请选择证件照片（.jpg.png.bmp.jpeg）！", "error", "");
            ModifyTipCss("u_licence_tip", "请选择证件照片（.jpg.png.bmp.jpeg）！");
...
function gettepi(obj) {
    window.parent.ShouwDiaLogWan("开发者特批", 480, 450, "/AppUser/AppUserTePi?userid=" + obj);
}
...
function UpdateRiskview(obj) {
    window.parent.ShouwDiaLogWan("风控资料", 480, 450, "/AppUser/AppUserRiskSh?userid=" + obj);
}
function UpateRisk() {
    var url = "/AppUser/CheckRiskSH";
    var IsSignContract = $('input[name="IsSignContract"]:checked').val();
    var IsRecord = $('input[name="IsRecord"]:checked').val();
    var id = $("#userid").val();
    var data = { IsSignContract: IsSignContract, IsRecord: IsRecord, userid: id };
    $.post(url, data, function (result) {
...
//重置开发者提现密码
function UpdateRePassword(obj) {
    //询问框
    window.parent.layer.confirm('确定重置提现密码吗?', {
        btn: ['确定', '取消'] //按钮
    }, function () {
        var url = "/AppUser/AppRePassword";
        var data = { id: obj };
