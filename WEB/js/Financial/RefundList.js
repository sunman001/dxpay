...
//退款管理
function AddAPPlog() {
    window.parent.ShouwDiaLogWan("退款申请", 745, 600, "/Financial/RefundAdd");
}
//退款人姓名验证
function yzname() {
    var exp = /^[\u4E00-\u9FA5]{1,6}$/;
...
function UpdateRefund(r_id) {
    ...
    window.parent.ShouwDiaLogWan("修改退款申请", 745, 600, "/Financial/UpdateRefund?r_id=" + r_id);
}
//弹窗开发者列表
function xzuser() {
...
function bulkassign() {
    ...
    window.parent.ShouwDiaLogWan("审核退款信息", 660, 380, "/Financial/AuditorRefund?rid=" + vals);
}
//单个审核
function auditorRefund(rid) {
    ...
    window.parent.ShouwDiaLogWan("审核退款信息", 660, 380, "/Financial/AuditorRefund?rid=" + rid);
}


function yzyymc(a_id, a_name, a_user_id, u_realname, index) {

    window.parent.layer.getChildFrame("#appName", index).val(a_name);
    window.parent.layer.getChildFrame("#r_appname", index).val(a_name);
    window.parent.layer.getChildFrame("#r_appid", index).val(a_id);
    window.parent.layer.getChildFrame("#a_user_idyx", index).val(u_realname);
    window.parent.layer.getChildFrame("#a_user_id", index).val(a_user_id);
    window.parent.layer.getChildFrame("#yzkfz", index).attr("class", "Validform_checktip  Validform_right");
    window.parent.layer.getChildFrame("#yzkfz", index).html("验证通过");
    var indexs = parent.layer.getFrameIndex(window.name);
    window.top.layer.close(indexs);
