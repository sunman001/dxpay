    ...
    $("#btnComplaintCL").click(function () {
        ...
        $.post("/REPORT/AppRequestAuditJG", { rid: uids, remark: c_result }, function (retJson) {
            if (retJson.success == 1) {
                window.location.href = "/REPORT/AppRequestAuditList";
...
function bulkassign() {
    ...
    window.parent.ShouwDiaLogWan("处理应用异常", 700, 380, "/REPORT/AppRequestAuditCL?rid=" + vals);
}
//单个审核
function complaintLC(rid) {
    ...
    window.parent.ShouwDiaLogWan("处理应用异常", 700, 380, "/REPORT/AppRequestAuditCL?rid=" + rid);
}
