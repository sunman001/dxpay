...
function AddLimitDialog() {
    window.parent.ShouwDiaLogWan("添加权限", 800, 500, "/LIMIT/InsertUserLimit");
}
...
function UpdateLimitDialog(objId) {
    window.parent.ShouwDiaLogWan("更新权限", 800, 500, "/LIMIT/UserUpdateLimit?lid=" + objId);
};
...
function addlcoaluser() {
    ...
    if (tmsg == "") {
        $.ajax({
            ...
            data: "name=" + name + "&pwd=" + pwd + "&realName=" + realname + "&department=" + department + "&position=" + position + "&roteId=" + roteId + "&state=" + state,
            success: function (retJson) {
                if (retJson.success == 1) {
                  window.parent.ShowMsg(retJson.msg, "ok", function () { window.parent.location.reload(); window.parent.layer.closeAll(); });
                }
...
function updatelcoaluser() {
    ...
    if (tmsg == "") {
        $.ajax({
            ...
            data: "name=" + name + "&pwd=" + pwd + "&realName=" + realname + "&department=" + department + "&position=" + position + "&roteId=" + roteId + "&state=" + state + "&id=" + uid,
            success: function (retJson) {
                if (retJson.success == 1) {
...
function AddRoleDialog() {
    window.parent.ShouwDiaLogWan("添加角色", 800, 250, "/ROTE/AddUserRote");

}

function UpdateRoleDialog(objId) {
    window.parent.ShouwDiaLogWan("编辑角色", 800, 250, "/ROTE/RoleUserUpdate?rid=" + objId);
}

//设置权限弹出框
function UpdateRoleLimitDialog(objId) {
    window.parent.ShouwDiaLogWan("编辑角色权限", 800, 600, "/ROTE/UpdateRoleLimit?type=1&rid=" + objId);
}
