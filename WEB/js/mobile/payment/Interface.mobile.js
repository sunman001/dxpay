        ...
        $.post(url, data, function (retJson) {
            ...
            if (retJson.success == 1) {
                window.parent.ShowMsg(retJson.msg, "ok", function () {
                    window.location.href = "/payment/InterfaceList";
                });
            }
...
function ArticleManage(pageIndex, pageSize) {
    ...
    url += "&type=" + searchType + "&sea_name=" + sea_name + "&SelectState=" + SelectState + "&searchDesc=" + searchDesc + "&auditstate=" + auditstate + "&risk=" + risk;
...
function apptc() {
    ...
    switch (l_risk) {
        ...
        case "0":
            $("#l_riskyy").attr("class", "Validform_checktip  Validform_right");
            $("#l_riskyy").html("验证通过");
            window.parent.ShouwDiaLogWan("选择应用", 1000, 700, "/payment/RisklevelListTc?appstr=" + appid);
            break;
        default:
            $("#l_riskyy").attr("class", "Validform_checktip Validform_wrong");
            $("#l_riskyy").html("请选择风控类型！");
...
function AddInterface() {
    window.location.href = "/payment/InterfaceAdd";

}
//修改支付配置弹窗
function UpdateInterface(lid) {
    window.location.href = "/payment/InterfaceAdd?lid=" + lid;
}
//设置支付配置为可用
...
function selectxzuser(index) {
    var valArr = new Array;
    $("#apptable").find("input[type='checkbox']:checked").each(function (i) {
        valArr[i] = $(this).val();
    });
