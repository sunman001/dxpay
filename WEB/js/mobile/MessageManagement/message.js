    ...
    $("#btnSaveAddmessage").click(function () {
        ...
        $.post(url, data, function (retJson) {
            if (retJson.success == 1) {
                ShowMsg(retJson.msg, "ok", function () { window.location.href = "/MessageManagement/messagelist";});
            }
    ...
    $("#btnSaveRemessage").click(function () {
        ...
        if ($.trim(contentname) != "") {
            if (!tszf.test(contentname)) {
                ShowMsg("消息内容不输入特殊字符", "error", "");
                return false;
            }
        } else {
            ShowMsg("消息内容不能为空", "error", "");
            return false;
        }
        ...
        $.post(url, data, function (retJson) {
            if (retJson.success == 1) {
                ShowMsg(retJson.msg, "ok", function () { window.top.location.href = "/MessageManagement/messagelist"; });
            }
            else if (retJson.success == 9998) {
                ShowMsg(retJson.msg, "error", "");
                return false;
...
//发布消息弹窗
function addmessage() {
    window.location.href = "/MessageManagement/message?type=dd";
}
//修改消息弹窗
function Updatemessage(mid) {
    window.location.href = "/MessageManagement/message?m_id=" + mid + "&type=up";
}
...
//修改选择
function UpdateUser() {
    ShouwDiaLogWan("选择开发者", 1000, 700, "/MessageManagement/UserList");
}
//选择开发者弹窗
function xzuser() {
    ShouwDiaLogWan("选择开发者", 1000, 700, "/MessageManagement/UserList");
...
//获取选择的用户（用于添加）
function selectxzuser() {
    var valArr = [];
    $("#tableUser").find('input[name="child"]:checked').each(function (i) {
        valArr[i] = $(this).val();
    });
    ...
    parent.$('#a_user_id').val(vals);
    parent.$('#yzkfz').attr("class", "Validform_checktip  Validform_right");
    parent.$('#yzkfz').html("验证通过");
    var indexs = parent.layer.getFrameIndex(window.name);
    window.top.layer.close(indexs);
    parent.document.getElementById("a_user_id").value=vals;
}
//验证公告内容
...
function UpdatestateMessage(state) {
    ...
    $.post(url, data, function (retJson) {
        if (retJson.success == 1) {
            ShowMsg(retJson.msg, "ok", function () { window.location.reload(); });
        }
        else if (retJson.success == 9998) {
            ShowMsg(retJson.msg, "error", "");
            return false;
