$(function () {
    //添加或修改支付类型
    $("#btnSaveAddorUpdatepaymode").click(function () {
        var zzyz = /^[\w\W]{1,20}$/;
        ...
        //    }
        //}
        //var interfa = $.trim($("#interface").val());
        //if ($.trim(interfa) != "") {
        //    $("#interfaceYz").attr("class", "Validform_checktip  Validform_right");
        //    $("#interfaceYz").html("验证通过");
        //} else {
        //    $("#interfaceYz").attr("class", "Validform_checktip Validform_wrong");
        //    $("#interfaceYz").html("请选择支付配置！");
        //    return false;
        //}
        var paymodeid = $.trim($("#paymodeid").val());
        $("#btnSaveAddorUpdatepaymode").attr("disabled", "disabled");
        var data = { p_name: $.trim(PaymodeName), p_rate: $.trim(0), p_id: $.trim(paymodeid) };
        var url = "/System/InsertOrUpdatePaymode";
        $.post(url, data, function (retJson) {
            ...
            $("#btnSaveAddorUpdatepaymode").attr("disabled", false);
        })
    })
