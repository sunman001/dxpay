...
function AddLimitDialog() {
    window.parent.ShouwDiaLogWan("添加权限", 950, 600, "/LIMIT/AddLimit");
}
...
function addlimits() {
    ...
    var count = nodes.length;
       topid = nodes[count-1].id; //获取选中节点的值
       pid = nodes[count-1].pId;
    console.log(topid);
...
function updateLimitTopid() {
    var sellimit_id = $("#sellimit_id").val();
    if (sellimit_id > 0) {
        var data = { sellimit_id: $.trim(sellimit_id) };
        var url = "/LIMIT/SelectLimitChirdID";
        $.post(url, data, function (msg) {
            $("#sellimit_chirdid").html(msg);
...
function UpdateLimitDialog(objId) {
    window.parent.ShouwDiaLogWan("更新权限", 950, 600, "/LIMIT/UpdateLimit?lid=" + objId);
};
...
function updatelimits(objid) {
    ...
    var treeObj = $.fn.zTree.getZTreeObj("treeDemo"),
           ...
           v = "";
    var count = nodes.length;
        topid = nodes[count - 1].id
       var pId  = nodes[count - 1].pId;
    var state = $('#radOrvActnS input[name="state"]:checked ').val();
    var icon = $.trim($("#inputIcnn").val());
    if (isNotNull(name)) {
        $("#checkName").attr("class", "Validform_checktip Validform_wrong");
...
function AddLocuserDialog() {
    window.parent.ShouwDiaLogWan("添加管理员", 800, 550, "/LOCUSER/AddUsers");
}

//编辑管理员弹出页面
function UpdateUser(objId) {
    window.parent.ShouwDiaLogWan("编辑管理员", 800, 550, "/LOCUSER/UpdateUsers?u_id=" + objId);
}
...
function AddRoleDialog() {
    window.parent.ShouwDiaLogWan("添加角色", 650, 240, "/ROTE/AddRole");

}

function UpdateRoleDialog(objId) {
    window.parent.ShouwDiaLogWan("编辑角色", 650, 240, "/ROTE/RoleUpdate?rid=" + objId);
}
