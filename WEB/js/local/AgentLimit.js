...
function AddLimitDialog() {
    ...
    window.parent.ShouwDiaLogWan("添加权限", 600, 500, "/LIMIT/InsertAgentLimit");
}
...
function UpdateLimitDialog(objId) {
    window.parent.ShouwDiaLogWan("更新权限", 600, 500, "/LIMIT/AgentUpdateLimit?lid=" + objId);
};
...
function updatelimits(objid) {
    ...
    $.ajax({
        cache: "False",
        type: "POST",
        url: "/LIMIT/AgentAjaxUpdateLimit",
...
function AddRoleDialog() {
    window.parent.ShouwDiaLogWan("添加角色", 600, 250, "/ROTE/AddAgentRote");

}

function UpdateRoleDialog(objId) {
    window.parent.ShouwDiaLogWan("编辑角色", 600, 250, "/ROTE/RoleAgentUpdate?rid=" + objId);
}

//设置权限弹出框
function UpdateRoleLimitDialog(objId) {
    window.parent.ShouwDiaLogWan("编辑角色权限", 800, 500, "/ROTE/UpdateRoleLimit?type=3&rid=" + objId);
}
