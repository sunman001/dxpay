...
namespace WEB.Extensions
{
    ...
    public class EntityModifiedTracker
    {
        ...
        public string Message
        {
            get
            {
                return string.Join(",", _modifiedProperties.Select(x => string.Format("{0}:{1}从{2}修改为:{3}", string.IsNullOrEmpty(x.Label) ? x.PropertyName : x.Label, "[" + x.PropertyName + "]", x.OldValue, x.NewValue)));
            }
        }
    }
    ...
    public static class CompareExtensions
    {
        ...
        public static EntityModifiedTracker GetModifiedTracker<T>(this T distObject, T sourceObject)
        {
            ...
            foreach (var disProp in disProps)
            {
                ...
                if (disValue != null && disValue.Equals(sourceValue))
                {
                    continue;
                }
                var c = new Modified
